...
def str2list(r):
    ...
    if isinstance(r, list) or type(r) == int or type(r) == float:
        ...
        if r.find('[') != -1:
            if r.find(',') != -1:
                values = list(map(int, r.strip('[').strip(']').split(',')))
            else:
                values = list(map(int, r.strip('[').strip(']').split()))
        else:
            values = int(r)
...
def plot_points_range(model, X, Y, npts=10000, colors=['r', 'g', 'b', 'y', 'l', 'k']):
    ...
    # send the transpose of X during evaluation as well...
    cs = model.predict(tx)

    uclasses = list(np.unique(cs))
    print(uclasses)
    ax.scatter(tx[:, 0], tx[:, 1], c=list(
        [colors[uclasses.index(x)] for x in cs]), alpha=.25)
    ax.legend([str(c) for c in uclasses])

    ax.hold(True)
    ax.scatter(X[:, 0], X[:, 1],
               c=list([colors[uclasses.index(x)] for x in Y]),
